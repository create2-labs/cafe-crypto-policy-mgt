...
**CP-PERSIST (EOA, no drafts — ADR 2026-08-24 / RD-P1 contract):** clients **must** obtain the canonical authorization message from CPM via `POST /api/cpm/v1/wallet-challenges` (stateless; request includes hashed `payload`; CPM computes `payload_sha256`) before signing. Normative persist is **`POST /api/cpm/v1/policies`** with `signed_message` + `signature` + the same `payload`. **No public `/drafts*`** in the OpenAPI contract (handlers may lag until RD-P5 — contrat ahead of runtime). Shared JCS vectors: [`internal/contract/testdata/payload_sha256/`](./internal/contract/testdata/payload_sha256/). Spec: [`docs/CP_PERSIST.md`](./docs/CP_PERSIST.md) v1.0.0. **No Redis** / `ProofStore` in this model. Session auth (JWT) and wallet signature are orthogonal.
