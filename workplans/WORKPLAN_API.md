...
**CP-PERSIST (wallet control proof, EOA) :** [`docs/CP_PERSIST.md`](../docs/CP_PERSIST.md) — **section « Current normative contract v1.0.0 — no drafts »** = source de vérité. Engagement = signed `POST /api/cpm/v1/policies` + `payload_sha256` (JCS). Helper `POST /wallet-challenges` stateless. **Plus de** `/drafts*`. Historical Parts below in CP_PERSIST may still mention drafts for archaeology.

**Remove CP drafts (ADR 2026-08-24) — Validé :** [`cafe-adr/ADR_20260824_remove_cp_drafts.md`](https://github.com/create2-labs/cafe-adr/blob/main/ADR_20260824_remove_cp_drafts.md) — exécution [`ADR_20260824_remove_cp_drafts_PR_PLAN.md`](https://github.com/create2-labs/cafe-adr/blob/main/ADR_20260824_remove_cp_drafts_PR_PLAN.md) (**RD-P\***). **RD-P1–P13** ✅. **RD-P14** : amend IMM/TODO FE + ADR persistence §8.4. Cible runtime : plus de `/drafts*` ; engagement = `POST /policies` signé + `payload_sha256` ; W2 partout. **§2.2 W1b / routes draft / §4.4.1 Platform drafts** ci-dessous = **archéologie** — ne plus traiter comme contrat vivant (voir `CP_PERSIST.md` v1.0.0 + OpenAPI).
---
