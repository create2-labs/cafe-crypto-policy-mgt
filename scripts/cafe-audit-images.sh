...
REPORT_FILE="${REPORT_FILE:-$REPORT_DIR/cafe-cpm-security-audit-$RUN_ID.md}"
...
run_gov() {
  if ! have govulncheck; then state_set gov "absent"; return; fi
  local out
...
main() {
  ...
  fi
  {
    ...
    echo "## Statique: linter: $(state_get lint | tr '|' /) — govulncheck: $(state_get gov | tr '|' /)"
    echo ""
